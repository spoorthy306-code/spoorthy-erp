...
class AccountingService:
    ...
    def post_order_to_ledger(db: Session, order):
        ...
        sales_ledger = (
            db.query(Ledger)
            .filter(Ledger.name.in_(["Sales Account", "Profit & Loss A/c"]))
            .first()
        )
        tax_ledger = (
            db.query(Ledger)
            .filter(Ledger.name.in_(["Duties & Taxes", "Tax Account"]))
            .first()
        )
        debtors_ledger = (
            db.query(Ledger)
            .filter(Ledger.name.in_(["Sundry Debtors", "Cash Account"]))
            .first()
        )

        if not sales_ledger or not tax_ledger or not debtors_ledger:
            raise ValueError(
                "Required ledgers not found for accounting post: sales=%s tax=%s debtors=%s"
                % (bool(sales_ledger), bool(tax_ledger), bool(debtors_ledger))
            )
