...
class InvoiceService:
    ...
    def generate_pdf(order, filename: str = None):
        ...
        c.drawString(1 * cm, height - 2 * cm, company.name if company else "SPOORTHY ERP")
        ...
        if company and company.logo_path and os.path.exists(company.logo_path):
            c.drawImage(company.logo_path, width - 4.5 * cm, height - 4.5 * cm, width=3 * cm, preserveAspectRatio=True)

        # Invoice metadata
        c.setFont("Helvetica-Bold", 14)
        c.drawRightString(width - 1 * cm, height - 2 * cm, f"INVOICE: INV-{order.id}")
        c.setFont("Helvetica", 10)
        c.drawRightString(width - 1 * cm, height - 2.6 * cm, f"Date: {order.created_at.strftime('%Y-%m-%d')}")
        ...
        c.drawRightString(width - 1.5 * cm, y - 1.5 * cm, f"{float(order.total_amount):.2f}")

        # Totals
        c.line(1 * cm, y - 2.4 * cm, width - 1 * cm, y - 2.4 * cm)
        c.drawString(12 * cm, y - 3.0 * cm, "Subtotal:")
        c.drawRightString(width - 1.5 * cm, y - 3.0 * cm, f"{float(order.subtotal):.2f}")
        c.drawString(12 * cm, y - 3.5 * cm, "Tax Amount:")
        c.drawRightString(width - 1.5 * cm, y - 3.5 * cm, f"{float(order.tax_amount):.2f}")

        c.setFont("Helvetica-Bold", 12)
        c.drawString(12 * cm, y - 4.1 * cm, "GRAND TOTAL:")
        c.drawRightString(width - 1.5 * cm, y - 4.1 * cm, f"INR {float(order.total_amount):.2f}")
