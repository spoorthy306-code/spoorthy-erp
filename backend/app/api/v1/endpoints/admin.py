...
async def get_quantum_stats(db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "total_solve_time_ms": sum(
            job.solve_time_ms or 0
            for job in reconciliation_jobs + forecasting_jobs + optimization_jobs
        ),
        "average_energy": sum(
            job.energy or 0
            for job in reconciliation_jobs + forecasting_jobs + optimization_jobs
        )
        / max(1, len(reconciliation_jobs + forecasting_jobs + optimization_jobs)),
    }


@router.post("/backup/{entity_id}")
async def create_backup(entity_id: UUID, background_tasks: BackgroundTasks):
...
async def run_migrations(background_tasks: BackgroundTasks):
