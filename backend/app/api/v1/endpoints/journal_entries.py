...
from ....repositories.repositories import JournalEntryRepository, BankTransactionRepository
from ....models.models import JournalEntrySchema, JournalEntryCreateSchema, JournalLine
...
async def list_journal_entries(
    ...
    if period:
        ...
        entries = await repo.get_all(skip=skip, limit=limit, filters={"entity_id": entity_id})
...
async def run_quantum_reconciliation(entity_id: UUID):
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).exception("Quantum reconciliation failed for entity %s", entity_id)