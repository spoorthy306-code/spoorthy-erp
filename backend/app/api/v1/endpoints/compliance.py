...
async def generate_gstr1(
    ...
    await repo.create(
        entity_id=entity_id, return_type="GSTR1", period=period, json_payload=gstr1_data
    )
...
async def get_gst_returns(
    entity_id: UUID, return_type: str = None, db: AsyncSession = Depends(get_db)
):
...
async def get_tds_summary(
    entity_id: UUID, period: str, db: AsyncSession = Depends(get_db)
):
...
async def get_compliance_status(entity_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    return {
        ...
        "overall_status": (
            "compliant"
            if gst_status["compliant"] and tds_status["compliant"]
            else "non_compliant"
        ),
    }
...
async def generate_itr(
    entity_id: UUID, assessment_year: str, db: AsyncSession = Depends(get_db)
):
...
async def get_regulatory_reports(
    entity_id: UUID, report_type: str, period: str, db: AsyncSession = Depends(get_db)
):
...
async def auto_file_gst_return(
    entity_id: UUID, return_type: str, period: str, db: AsyncSession
):
