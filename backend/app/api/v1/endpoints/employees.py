...
from ....models.models import Employee, EmployeeCreateSchema, EmployeeSchema
...
async def create_employee(
    employee: EmployeeCreateSchema, db: AsyncSession = Depends(get_db)
):
...
async def delete_employee(employee_id: UUID, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        delete(Employee).where(Employee.employee_id == employee_id)
    )
    await db.commit()
