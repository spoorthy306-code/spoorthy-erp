...
from ....models.models import Loan, LoanCreateSchema, LoanSchema
...
async def create_loan(loan: LoanCreateSchema, db: AsyncSession = Depends(get_db)):
...
async def update_loan(
    loan_id: UUID, loan_update: LoanCreateSchema, db: AsyncSession = Depends(get_db)
):
...
async def close_loan(loan_id: UUID, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        update(Loan)
        .where(Loan.loan_id == loan_id)
...
async def delete_loan(loan_id: UUID, db: AsyncSession = Depends(get_db)):
