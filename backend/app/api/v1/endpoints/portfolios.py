...
from ....models.models import Portfolio, PortfolioCreateSchema, PortfolioSchema
...
async def create_portfolio(
    portfolio: PortfolioCreateSchema, db: AsyncSession = Depends(get_db)
):
    data = portfolio.model_dump()
    holdings = data.get("holdings") or {}
    data["total_value"] = sum(
        h.get("value", 0) for h in holdings.values() if isinstance(h, dict)
    )
...
async def delete_portfolio(portfolio_id: UUID, db: AsyncSession = Depends(get_db)):
    result = await db.execute(
        delete(Portfolio).where(Portfolio.portfolio_id == portfolio_id)
    )
    await db.commit()
