...
async def get_chart_of_accounts(
    entity_id: UUID, active_only: bool = Query(True), db: AsyncSession = Depends(get_db)
):
...
async def get_accounts_hierarchy(entity_id: UUID, db: AsyncSession = Depends(get_db)):
