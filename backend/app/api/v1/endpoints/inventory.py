...
from ....models.models import Inventory, InventoryCreateSchema, InventorySchema
...
async def create_inventory_item(
    item: InventoryCreateSchema, db: AsyncSession = Depends(get_db)
):
...
async def get_inventory_item(sku: str, db: AsyncSession = Depends(get_db)):
...
async def update_inventory_item(
    sku: str, item_update: InventoryCreateSchema, db: AsyncSession = Depends(get_db)
):
...
async def adjust_inventory_qty(
    sku: str, qty_change: float, db: AsyncSession = Depends(get_db)
):
    repo = InventoryRepository(db)
...
async def delete_inventory_item(sku: str, db: AsyncSession = Depends(get_db)):
