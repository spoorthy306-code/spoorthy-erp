...
from ....models.models import BankTransactionSchema, BankTransactionCreateSchema

router = APIRouter()

@router.post("/", response_model=BankTransactionSchema, status_code=201)
async def create_bank_transaction(
    txn: BankTransactionCreateSchema,
...
async def get_bank_transaction(
    ...
    result = await db.execute(select(BankTransaction).where(BankTransaction.txn_id == txn_id))
...
async def reconcile_transaction(
    ...
    return {"message": "Transaction reconciled", "txn_id": str(txn_id), "entry_id": str(entry_id)}
...
async def delete_bank_transaction(
    ...
    from ....models.models import BankTransaction
    result = await db.execute(delete(BankTransaction).where(BankTransaction.txn_id == txn_id))
    await db.commit()
