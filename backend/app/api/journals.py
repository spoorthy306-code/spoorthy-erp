...
from backend.app.schemas.finance import JournalCreate, JournalRead, JournalLineBase
...
def create_journal(entry: JournalCreate, db: Session = Depends(get_db), _: object = Depends(require_roles(['Admin', 'Accountant']))):
    if not entry.transactions or len(entry.transactions) < 2:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='At least two transaction lines are required for double-entry')

    total_debit, total_credit = _calculate_totals(entry.transactions)
    if total_debit <= 0 or total_credit <= 0 or abs(total_debit - total_credit) > 0.0001:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Debits and credits must match and be > 0')
    ...
    if count != len(ledger_ids):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='One or more ledgers not found')
...
def list_journals(start_date: Optional[str] = None, end_date: Optional[str] = None, status_filter: Optional[str] = None, db: Session = Depends(get_db), _: object = Depends(get_current_user)):
...
def update_journal(journal_id: int, entry: JournalCreate, db: Session = Depends(get_db), _: object = Depends(require_roles(['Admin', 'Accountant']))):
    journal = db.query(Journal).get(journal_id)
    if not journal:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Journal entry not found')

    if journal.posted:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Cannot edit posted journal entry')

    total_debit, total_credit = _calculate_totals(entry.transactions)
    if abs(total_debit - total_credit) > 0.0001:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Debits and credits must match')
...
def post_journal(journal_id: int, db: Session = Depends(get_db), _: object = Depends(require_roles(['Admin', 'Accountant']))):
    journal = db.query(Journal).get(journal_id)
    if not journal:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail='Journal entry not found')
