...
from sqlalchemy import (DECIMAL, JSON, UUID, Boolean, Column, Date, DateTime,
...
class ChartOfAccount(Base):
    ...
    entity_id: Mapped[uuid.UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("entities.entity_id")
    )
...
class JournalEntry(Base):
    ...
    bank_transactions: Mapped[List["BankTransaction"]] = relationship(
        back_populates="reconciled_entry"
    )
...
class JournalLine(Base):
    ...
    entry_id: Mapped[uuid.UUID] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("journal_entries.entry_id")
    )
    account_code: Mapped[str] = mapped_column(
        String(20), ForeignKey("chart_of_accounts.account_code")
    )
...
class BankTransaction(Base):
    ...
    reconciled_entry_id: Mapped[Optional[uuid.UUID]] = mapped_column(
        PGUUID(as_uuid=True), ForeignKey("journal_entries.entry_id")
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=func.now()
    ...
    reconciled_entry: Mapped[Optional["JournalEntry"]] = relationship(
...
class EntityCreateSchema(BaseModel):
    ...
    gstin: Optional[str] = Field(
        None, pattern=r"^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$"
    )
