...
from backend.app.models import Entity, GSTReturn, Invoice, JournalEntry
...
def init():
    ...
    try:
        ...
        click.echo(
            click.style("✓ Database schema initialized successfully", fg="green")
        )
            ...
            try:
                ...
                if entity_id:
                    ...
                    if entity:
                        ...
                        click.echo(
                            click.style(f"Entity {entity_id} not found", fg="yellow")
                        )

            except Exception as e:
                click.echo(
                    click.style(f"Error checking database status: {e}", fg="red")
                )
...
def optimize_portfolio(entity_id, risk_profile):
    ...
    click.echo(
        f"Running quantum portfolio optimization for entity {entity_id} ({risk_profile})..."
    )
...
@click.option(
    "--statement",
    type=click.Choice(["pnl", "balance", "cashflow"]),
    required=True,
    help="Statement type",
)
def generate_statement(entity_id, period, statement):
    ...
    click.echo(
        f"Generating {statement.upper()} statement for {entity_id} - {period}..."
    )
    ...
    async def _generate():
        try:
            ...
            if statement == "pnl":
                ...
                click.echo(
                    click.style("✓ Profit & Loss statement generated", fg="green")
                )
                ...
                click.echo(
                    f"  Total Liabilities: ₹{result.get('total_liabilities', 0):,.0f}"
                )
                ...
                click.echo(
                    f"  Operating Cash Flow: ₹{result.get('operating_cash_flow', 0):,.0f}"
                )
...
@click.option(
    "--return-type",
    type=click.Choice(["gstr1", "gstr3b"]),
    required=True,
    help="GST return type",
)
    ...
    async def _generate_gst():
        try:
            ...
            if return_type == "gstr1":
                ...
                click.echo(
                    f"  Input tax credit: ₹{result.get('input_tax_credit', 0):,.0f}"
                )
                click.echo(
                    f"  Net tax payable: ₹{result.get('net_tax_payable', 0):,.0f}"
                )
    ...
    async def _file_gst():
        try:
            ...
            if result["success"]:
                ...
                click.echo(
                    click.style(
                        f"✗ GST return filing failed: {result.get('error', 'Unknown error')}",
                        fg="red",
                    )
                )
        ...
        try:
            ...
            if output:
                with open(output, "w" if format != "excel" else "wb") as f:
                    f.write(output_data)
                click.echo(
                    click.style(f"✓ Trial balance exported to {output}", fg="green")
                )
            else:
                ...
                click.echo(
                    click.style(
                        f"✓ Trial balance exported to {default_filename}", fg="green"
                    )
                )
...
def journal_report(entity_id, start_date, end_date):
    ...
    click.echo(
        f"Generating journal report for {entity_id} from {start_date} to {end_date}..."
    )
    ...
    async def _journal_report():
        try:
            async for session in get_db():
                ...
                click.echo(
                    click.style(f"✓ Found {len(entries)} journal entries", fg="green")
                )

                for entry in entries[:10]:  # Show first 10
                    click.echo(
                        f"  {entry.date} - {entry.description}: ₹{entry.amount:,.0f}"
                    )
...
def health():
    ...
    for check_name, check_result in health["checks"].items():
        ...
        click.echo(
            click.style(f"  {check_name}: {check_result['status']}", fg=status_color)
        )
...
def metrics():
    ...
    if metrics["quantum_jobs"]:
        ...
        click.echo(
            f"Latest quantum job: {latest_job['job_type']} ({latest_job['solve_time_ms']:.1f}ms)"
        )
...
def alerts():
    ...
    if not alerts:
        ...
        for alert in alerts:
            ...
            click.echo(
                click.style(
                    f"  {alert['alertname']} ({alert['severity']}): {alert['description']}",
                    fg=severity_color,
                )
            )
