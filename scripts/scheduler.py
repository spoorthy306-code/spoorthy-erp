            ...
            async for session in get_db():
                ...
                for entity in entities:
                    try:
                        ...
                        needs_reconciliation = await self._check_reconciliation_needed(entity.id)

                        if needs_reconciliation:
                            await self._run_entity_reconciliation(entity.id)
                            logger.info("reconciliation_completed", entity_id=entity.id)
                        else:
                            logger.debug("reconciliation_not_needed", entity_id=entity.id)

                    except Exception as e:
                        logger.error("entity_reconciliation_failed",
            ...
            async for session in get_db():
                ...
                for entity in entities:
                    try:
                        ...
                        compliance_status = await gst_engine.check_compliance_status(entity.id, period)

                        if compliance_status['needs_filing']:
                            logger.warning("gst_return_needs_filing",
                                         entity_id=entity.id, period=period)
                            # Could send notification here
                        else:
                            logger.info("gst_compliance_ok", entity_id=entity.id, period=period)

                    except Exception as e:
                        logger.error("entity_gst_check_failed",
                                   entity_id=entity.id, error=str(e))
                        self.job_stats['failed'] += 1
            ...
            async for session in get_db():
                ...
                for entity in entities:
                    try:
                        ...
                        balance_sheet = await statement_gen.generate_balance_sheet(entity.id, period)
                        cash_flow = await statement_gen.generate_cash_flow(entity.id, period)

                        # Store or notify (simplified)
                        logger.info("financial_statements_generated",
                                  entity_id=entity.id, period=period)

                    except Exception as e:
                        logger.error("entity_financial_statements_failed",
                                   entity_id=entity.id, error=str(e))
                        self.job_stats['failed'] += 1
                ...
                for entity in entities:
                    try:
                        ...
                        if needs_rebalancing:
                            ...
                            logger.debug("portfolio_rebalancing_not_needed", entity_id=entity.id)

                    except Exception as e:
                        logger.error("entity_portfolio_rebalancing_failed",
                                   entity_id=entity.id, error=str(e))
                        self.job_stats['failed'] += 1
            ...
            async for session in get_db():
                ...
                for entity in entities:
                    try:
                        ...
                        compliance_score = await self._calculate_compliance_score(entity.id)
                        ...
                        if compliance_score < 80:
                            logger.warning("low_compliance_score",
                                         entity_id=entity.id, score=compliance_score)

                    except Exception as e:
                        logger.error("entity_compliance_monitoring_failed",
                                   entity_id=entity.id, error=str(e))
                        self.job_stats['failed'] += 1
