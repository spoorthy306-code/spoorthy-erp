...
class ExportManager:
    ...
    async def export_trial_balance(
        self, entity_id: str, period: str, format: str = "excel"
    ) -> str:
        """Export trial balance"""
        logger.info(
            "exporting_trial_balance", entity_id=entity_id, period=period, format=format
        )
    ...
    async def export_profit_loss(
        ...
        logger.info(
            "exporting_profit_loss", entity_id=entity_id, period=period, format=format
        )
    ...
    async def export_balance_sheet(
        ...
        logger.info(
            "exporting_balance_sheet", entity_id=entity_id, period=period, format=format
        )
    ...
    async def export_gst_return(
        self, entity_id: str, period: str, return_type: str, format: str = "excel"
    ) -> str:
        """Export GST return"""
        logger.info(
            "exporting_gst_return",
            entity_id=entity_id,
            period=period,
    ...
    async def export_journal_entries(
        self, entity_id: str, start_date: str, end_date: str, format: str = "excel"
    ) -> str:
        """Export journal entries"""
        logger.info(
            "exporting_journal_entries",
            entity_id=entity_id,
    ...
    async def export_aging_report(
        ...
        """Export aging report"""
        logger.info(
            "exporting_aging_report", entity_id=entity_id, period=period, format=format
        )
    ...
    async def export_comprehensive_report(self, entity_id: str, period: str) -> str:
        ...
        logger.info(
            "exporting_comprehensive_report", entity_id=entity_id, period=period
        )
    ...
    async def _export_pl_excel(self, data: Dict, filepath: str):
        ...
        with pd.ExcelWriter(filepath, engine="openpyxl") as writer:
            rows = []
            for item in data.get("items", []):
                rows.append({"Account": item["account"], "Amount": item["amount"]})
    ...
    async def _export_balance_sheet_excel(self, data: Dict, filepath: str):
        ...
        with pd.ExcelWriter(filepath, engine="openpyxl") as writer:
            ...
            for item in data.get("assets", []):
                asset_rows.append({"Asset": item["account"], "Amount": item["amount"]})
            ...
            for item in data.get("liabilities", []):
                liability_rows.append(
                    {"Liability/Equity": item["account"], "Amount": item["amount"]}
                )

            liabilities_df = pd.DataFrame(liability_rows)
            liabilities_df.to_excel(
                writer, sheet_name="Liabilities & Equity", index=False
            )
    ...
    async def _export_comprehensive_excel(self, data: Dict, filepath: str):
        ...
        with pd.ExcelWriter(filepath, engine="openpyxl") as writer:
            ...
            for item in data["profit_loss"].get("items", []):
                pl_rows.append({"Account": item["account"], "Amount": item["amount"]})
            pd.DataFrame(pl_rows).to_excel(
                writer, sheet_name="Profit & Loss", index=False
            )

            # Balance Sheet
            bs_assets = []
            for item in data["balance_sheet"].get("assets", []):
                bs_assets.append({"Asset": item["account"], "Amount": item["amount"]})
            ...
            pd.DataFrame(bs_liabilities).to_excel(
                writer, sheet_name="Liabilities", index=False
            )

            # Cash Flow
            cf_rows = []
            for item in data["cash_flow"].get("items", []):
                cf_rows.append({"Category": item["category"], "Amount": item["amount"]})
    ...
    async def _export_trial_balance_csv(self, data: Dict, filepath: str):
        ...
        with open(filepath, "w", newline="") as csvfile:
            ...
            for item in data.get("items", []):
                writer.writerow(
                    [item["account"], item.get("debit", 0), item.get("credit", 0)]
                )
    ...
    async def _export_pl_csv(self, data: Dict, filepath: str):
        ...
        with open(filepath, "w", newline="") as csvfile:
            ...
            for item in data.get("items", []):
                writer.writerow([item["account"], item["amount"]])
    ...
    async def _export_journal_csv(self, data: Dict, filepath: str):
        ...
        with open(filepath, "w", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow(
                ["ID", "Date", "Description", "Amount", "Type", "Reference", "Period"]
            )
...
async def export_gst_return(
    entity_id: str, period: str, return_type: str, format: str = "excel"
):
...
async def export_journal_entries(
    entity_id: str, start_date: str, end_date: str, format: str = "excel"
):
    """CLI function to export journal entries"""
    exporter = ExportManager()
    filepath = await exporter.export_journal_entries(
        entity_id, start_date, end_date, format
    )
